...
TestObject submitButton2 = new TestObject().addProperty("xpath", ConditionType.CONTAINS, "//button[contains(text(), 'Submit') and @type='submit']")
WebUI.click(submitButton2)

TestObject submitButton3 = new TestObject().addProperty("xpath", ConditionType.CONTAINS, "//button[contains(text(), 'Submit') and @type='submit']")
WebUI.click(submitButton3)


TestObject errorMessage = new TestObject().addProperty("xpath", 
    com.kms.katalon.core.testobject.ConditionType.EQUALS, 
    "//div[@role='status' and contains(text(), 'Pin anda terblokir sementara')]")

// Tunggu elemen muncul (maksimal 10 detik)
WebUI.waitForElementVisible(errorMessage, 10)

// Verifikasi teks error
assert WebUI.verifyElementText(errorMessage, "Pin anda terblokir sementara, Silakan coba lagi dalam 5 menit")
